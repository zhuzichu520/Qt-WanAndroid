...
public:
    ...
    explicit QHotkey(Qt::Key keyCode, Qt::KeyboardModifiers modifiers, bool autoRegister = false,
    ...
    Q_SLOT bool setRegistered(bool registered);

    //! @writeAcFn{QHotkey::shortcut}
    Q_SLOT bool setShortcut(const QKeySequence &shortcut, bool autoRegister = false);
    //! @writeAcFn{QHotkey::shortcut}
    Q_SLOT bool setShortcut(Qt::Key keyCode, Qt::KeyboardModifiers modifiers, bool autoRegister = false);
    //! @resetAcFn{QHotkey::shortcut}
    Q_SLOT bool resetShortcut();
    ...
    Q_SIGNAL void activated(QPrivateSignal);

    //! Will be emitted if the shortcut press is released
    Q_SIGNAL void released(QPrivateSignal);

    //! @notifyAcFn{QHotkey::registered}
    Q_SIGNAL void registeredChanged(bool registered);
