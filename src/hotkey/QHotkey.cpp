...
void QHotkeyPrivate::addMappingInvoked(Qt::Key keycode, Qt::KeyboardModifiers modifiers,
    ...
    if (!shortcuts.contains(shortcut)) {
        if (!registerShortcut(shortcut)) {
            qCWarning(logQHotkey)
                << QHotkey::tr("Failed to register %1. Error: %2").arg(hotkey->shortcut().toString(), error);
            return false;
        }
    }
...
bool QHotkeyPrivate::removeShortcutInvoked(QHotkey *hotkey) {
    ...
    if (shortcuts.count(shortcut) == 0) {
        if (!unregisterShortcut(shortcut)) {
            qCWarning(logQHotkey)
                << QHotkey::tr("Failed to unregister %1. Error: %2").arg(hotkey->shortcut().toString(), error);
            return false;
        }
        return true;
    }
